...
const CompanyRegisterForm = () => {
  ...
  const validateStep = (step) => {
    switch(step) {
      case 1:
        if (!formData.companyName || !formData.email || !formData.representativeName || !formData.phoneNumber || 
  ...
  const submitForm = async () => {
    if (validateStep(4)) {
      try {
        ...
        // Upload documents
        if (formData.commercialRegisterFile instanceof File) {
          const result = await storageService.uploadCompanyDocument(formData.commercialRegisterFile);
          submitPayload.commercialRegisterFile = result;
        }

        if (formData.taxCardFile instanceof File) {
          const result = await storageService.uploadCompanyDocument(formData.taxCardFile);
          submitPayload.taxCardFile = result;
        }

        if (formData.businessLicenseFile instanceof File) {
          const result = await storageService.uploadCompanyDocument(formData.businessLicenseFile);
          submitPayload.businessLicenseFile = result;
        }

        if (formData.additionalDocsFiles && formData.additionalDocsFiles.length > 0) {
          const uploadedDocs = [];
          for (const file of formData.additionalDocsFiles) {
            if (file instanceof File) {
              const result = await storageService.uploadCompanyDocument(file);
              uploadedDocs.push(result);
            }
          }
          submitPayload.additionalDocsFiles = uploadedDocs;
        }

        await companyService.submitRegistration(formData.email, submitPayload);
        ...
        showErrorMessage(error.response?.data?.message || 'حدث خطأ أثناء تقديم الطلب. يرجى المحاولة مرة أخرى.');
      } finally {
            ...
            {currentStep === 1 && (
              <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 fade-in">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">البريد الإلكتروني للشركة *</label>
                    <input 
                      name="email"
                      type="email"
              ...
              <div className="flex gap-3 mr-auto flex-wrap">
                ...
                {currentStep < 4 ? (
                  ...
                  <button 
                    ...
                    className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors btn-bounce disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSubmitting ? (
                      <div className="flex items-center gap-2">
