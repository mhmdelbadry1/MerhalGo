    ...
    switch(step) {
      ...
      case 7:
        ...
        if (formData.howDidYouKnow === 'previous_client' && (!formData.referralClientName || !formData.referralClientPhone)) {
          alert('من فضلك أدخل بيانات العميل السابق');
          return false;
        }
        break;
    }
            ...
            {[1, 2, 3, 4, 5, 6, 7].map(step => (
              <div key={step} className="flex flex-col items-center text-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold mb-2 ${
                  currentStep >= step ? 'bg-primary text-white' : 'bg-gray-300 text-gray-500'
                }`}>
                  {step}
                </div>
