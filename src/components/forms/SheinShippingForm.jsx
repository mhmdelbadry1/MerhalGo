...
const SheinShippingFullForm = () => {
  ...
  const validateStep = (step) => {
    switch(step) {
      case 1:
        ...
        if (formData.clientType === 'أخرى' && !formData.clientTypeOther) { alert('حدد نوع العميل'); return false; }
        break;
      case 2:
        if (!formData.serviceType) { alert('اختر نوع الخدمة'); return false; }
        if (formData.serviceType === 'شحن+تخليص' && !formData.importCard) { alert('حدد إذا كان لديك بطاقة استيراد'); return false; }
        break;
      case 3:
        if (!formData.orderNumber || !formData.marketplace) { alert('أدخل بيانات الطلب'); return false; }
        if (formData.marketplace === 'Other' && !formData.marketplaceOther) { alert('اكتب اسم الموقع'); return false; }
        if (products.length === 0) { alert('أضف منتجًا واحدًا على الأقل'); return false; }
        for (let i = 0; i < products.length; i++) {
          const p = products[i];
          if (!p.link || !p.name || !p.qty || !p.price) {
            alert(`أكمل بيانات المنتج رقم ${i + 1}`); return false;
          }
        }
        break;
      case 4:
        if (!formData.shipMode) { alert('اختر طريقة الشحن'); return false; }
        break;
      case 5:
        if (!formData.hasHazmat || !formData.totalWeight) { alert('أكمل تفاصيل المنتجات'); return false; }
        if (formData.hasHazmat === 'نعم' && !formData.hazmatType) { alert('اذكر نوع المواد'); return false; }
        break;
      case 6:
        if (!formData.recvCountry || !formData.recvPort || !formData.recvAddress) {
          alert('أكمل بيانات التوصيل'); return false;
        }
        break;
      case 7:
        ...
        if (formData.howFound === 'referral' && (!formData.referralName || !formData.referralPhone)) {
          alert('أدخل بيانات العميل السابق'); return false;
        }
        break;
    }
    return true;
  ...
  const handleSubmit = (e) => {
    ...
    const order = {
      id: 'SHEIN-' + Date.now(), type: 'shein', status: 'pending',
      data: { ...formData, products }, createdAt: new Date().toISOString()
    };
  ...
  const addProduct = () => {
    if (products.length >= 10) { alert('الحد الأقصى 10 منتجات'); return; }
    setProducts([...products, { id: productCounter + 1, link: '', name: '', qty: '', price: '', note: '' }]);
    setProductCounter(productCounter + 1);
  };

  const removeProduct = (id) => {
    if (products.length === 1) { alert('يجب أن يكون هناك منتج واحد على الأقل'); return; }
