...
const ChineseStoresFullForm = () => {
    ...
    switch(step) {
      ...
      case 2:
        ...
        if (formData.serviceType === 'شحن+تخليص' && !formData.importCard) {
          alert('حدد إذا كان لديك بطاقة استيراد');
          return false;
        }
        break;
      case 3:
        ...
        for (let i = 0; i < products.length; i++) {
          const p = products[i];
          if (!p.link) {
            alert(`من فضلك أدخل رابط المنتج رقم ${i + 1}`);
            return false;
          }
          if (!p.name) {
            alert(`من فضلك أدخل اسم المنتج رقم ${i + 1}`);
            return false;
          }
          if (!p.qty || Number(p.qty) <= 0) {
            alert(`ادخل الكمية المطلوبة للمنتج رقم ${i + 1}`);
            return false;
          }
          if (p.hasColors === 'yes') {
            if (p.colors.length === 0) {
              alert(`أضف لونًا واحدًا على الأقل للمنتج رقم ${i + 1}`);
              return false;
            }
            for (let c of p.colors) {
              if (!c.name || !c.qty || Number(c.qty) <= 0) {
                alert(`أدخل اسم وكمية صحيحة لكل لون في المنتج رقم ${i + 1}`);
                return false;
              }
            }
          }
          if (p.hasSizes === 'yes') {
            if (p.sizes.length === 0) {
              alert(`أضف مقاسًا واحدًا على الأقل للمنتج رقم ${i + 1}`);
              return false;
            }
            for (let s of p.sizes) {
              if (!s.name || !s.qty || Number(s.qty) <= 0) {
                alert(`أدخل اسم وكمية صحيحة لكل مقاس في المنتج رقم ${i + 1}`);
                return false;
              }
            }
          }
  ...
  const handleSubmit = (e) => {
    ...
    const data = { ...formData, products, createdAt: new Date().toISOString() };
    const orders = JSON.parse(localStorage.getItem('mirhal_china_full_orders') || '[]');
    orders.unshift({ id: 'CHN-' + Date.now(), createdAt: new Date().toISOString(), data });
    localStorage.setItem('mirhal_china_full_orders', JSON.stringify(orders));
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm mb-1">رفع ملفات (فاتورة / صور / كتالوج) — اختياري</label>
                    <input
                      type="file"
                      multiple
