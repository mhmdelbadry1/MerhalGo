...
const InternationalShippingFullForm = () => {
  ...
  const portsData = {
    egypt: { بحري: ['الإسكندرية - ميناء الإسكندرية', 'بورسعيد - ميناء بورسعيد', 'دمياط - ميناء دمياط'], جوي: ['مطار القاهرة الدولي', 'مطار برج العرب'], بري: ['القاهرة', 'الإسكندرية'] },
    china: { بحري: ['ميناء شنغهاي', 'ميناء نينغبو', 'ميناء شنتشن'], جوي: ['مطار بكين', 'مطار شنغهاي'], بري: ['بكين', 'شنغهاي'] },
    usa: { بحري: ['ميناء لوس أنجلوس', 'ميناء نيويورك'], جوي: ['مطار JFK', 'مطار LAX'], بري: ['نيويورك', 'لوس أنجلوس'] }
  };
    ...
    switch(step) {
      case 1:
        if (!formData.clientName || !formData.phoneNumber || !formData.waNumber || !formData.clientType) {
          alert('أدخل جميع البيانات المطلوبة'); return false;
        }
        if (formData.clientType === 'other' && !formData.clientTypeOther) { alert('حدد نوع العميل'); return false; }
        break;
      case 2:
        if (!formData.operationType || !formData.cargoMode || !formData.serviceType) { alert('أكمل تفاصيل العملية'); return false; }
        break;
      case 3:
        if (!formData.pickupCountry || !formData.pickupPort || !formData.routeType) { alert('أكمل بيانات التحميل'); return false; }
        if ((formData.routeType === 'door-door' || formData.routeType === 'door-port') && !formData.pickupFromAddress) { alert('أدخل عنوان التحميل'); return false; }
        break;
      case 4:
        if (!formData.deliveryCountry || !formData.deliveryPort) { alert('أكمل بيانات التسليم'); return false; }
        if ((formData.routeType === 'door-door' || formData.routeType === 'port-door') && !formData.deliveryToAddress) { alert('أدخل عنوان التسليم'); return false; }
        break;
      case 5:
        if (!formData.goodsDescription || !formData.hasLiquids || !formData.hasHazmat || !formData.hasBatteries || !formData.isFragile || !formData.hasDocuments || !formData.totalWeight || !formData.packagingType) {
          alert('أكمل تفاصيل البضاعة'); return false;
        }
        if (formData.hasLiquids === 'نعم' && !formData.liquidType) { alert('حدد نوع السائل'); return false; }
        if (formData.hasHazmat === 'نعم' && !formData.hazmatType) { alert('حدد المادة الخطرة'); return false; }
        if (formData.hasBatteries === 'نعم' && !formData.batteryType) { alert('حدد نوع البطاريات'); return false; }
        if (formData.isFragile === 'نعم' && !formData.fragilePacking) { alert('حدد طريقة التغليف'); return false; }
        if (formData.hasDocuments === 'نعم' && !formData.docType) { alert('حدد نوع المستند'); return false; }
        if (formData.packagingType === 'أخرى' && !formData.packagingOther) { alert('حدد نوع التغليف'); return false; }
        break;
      case 6:
        if (formData.serviceType.includes('تخليص')) {
          if (!formData.clearanceLocation || !formData.shipmentPurpose) { alert('أكمل بيانات التخليص'); return false; }
          if (formData.shipmentPurpose === 'other' && !formData.shipmentPurposeOther) { alert('حدد الغرض'); return false; }
        }
        break;
      case 7:
        if (!formData.howFound) { alert('أخبرنا كيف علمت بنا'); return false; }
        if (formData.howFound === 'referral' && (!formData.referralName || !formData.referralPhone)) { alert('أدخل بيانات العميل السابق'); return false; }
        break;
    }
    return true;
  };
  ...
  const handleSubmit = (e) => {
    ...
    const order = { id: 'INTL-' + Date.now(), type: 'international', status: 'pending', data: formData, createdAt: new Date().toISOString() };
    const orders = JSON.parse(localStorage.getItem('mirhal_orders') || '[]');
    orders.unshift(order);
    localStorage.setItem('mirhal_orders', JSON.stringify(orders));
    localStorage.removeItem('mirhal_intl_draft');
    setShowSuccessModal(true);
  };

  const getPorts = (country, mode) => portsData[country]?.[mode] || [];
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <input value={formData.phoneNumber} onChange={(e) => setFormData({...formData, phoneNumber: e.target.value})} type="tel" className="flex-1 p-3 border rounded-lg" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <input value={formData.waNumber} onChange={(e) => setFormData({...formData, waNumber: e.target.value})} type="tel" className="flex-1 p-3 border rounded-lg" />
                    </div>
                  </div>
                  <div>
            ...
            <div className="flex justify-between items-center pt-6">
              ...
              <div className="flex gap-3">
                ...
                {currentStep < 7 ? (
                  ...
                  <button type="submit" className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700">إرسال</button>
                )}
              </div>
            </div>
          </form>
