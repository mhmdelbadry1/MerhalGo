...
const getRegistrationRequests = async (req, res) => {
  try {
    ...
    return sendSuccess(
      res,
      formatPaginatedResponse(requests, count, page, pageLimit),
    );
  } catch (error) {
    logger.error("Get registration requests error:", error);
    return sendError(res, "Failed to fetch requests", 500);
...
const approveCompany = async (req, res) => {
  try {
    ...
    logger.info(
      `approveCompany: initial password from req.body exists: ${!!password}`,
    );
    ...
    if (existingProfile) {
      logger.info(
        `User already exists for email ${email} (matches ${existingProfile.email}), checking role compatibility...`,
      );
      ...
      if (existingProfile.role === "admin") {
        logger.error(
          `Attempt to convert admin account to company blocked: ${email}`,
        );
        return sendError(
          res,
          "لا يمكن تحويل حساب المسؤول إلى شركة. يرجى استخدام بريد إلكتروني مختلف.",
          403,
        );
      }
      ...
      if (existingProfile.role !== "customer") {
        logger.error(
          `Invalid role conversion attempt: ${existingProfile.role} → company for ${email}`,
        );
        return sendError(
          res,
          "نوع الحساب غير متوافق مع التحويل إلى شركة.",
          400,
        );
      }
      ...
      const { error: updateUserError } =
        await supabaseAdmin.auth.admin.updateUserById(authUserId, authUpdates);
      ...
      if (authError) {
        ...
        if (authError.code === "email_exists") {
          return sendError(
            res,
            "User exists but profile missing. Please contact support.",
            422,
          );
        }
        ...
        .insert({
          ...
          phone:
            formData.phoneCode && formData.phoneNumber
              ? `${formData.phoneCode}${formData.phoneNumber}`
              : null,
          phone_country_code: formData.phoneCode || "+20",
          whatsapp:
            formData.whatsappCode && formData.whatsappNumber
              ? `${formData.whatsappCode}${formData.whatsappNumber}`
              : null,
          whatsapp_country_code: formData.whatsappCode || "+20",
          role: "company",
        });
    ...
    logger.info(
      `Sending approval email to ${email}, finalPassword exists: ${!!finalPassword}`,
    );
    try {
...
const rejectCompany = async (req, res) => {
  try {
    ...
    logger.info(
      `Company rejected: ${request.email} by admin ${req.user.email}`,
    );
...
const getAllCompanies = async (req, res) => {
  try {
    ...
    return sendSuccess(
      res,
      formatPaginatedResponse(companies, count, page, pageLimit),
    );
  } catch (error) {
    logger.error("Get all companies error:", error);
    return sendError(res, "Failed to fetch companies", 500);
  ...
  try {
    ...
    const [
      ...
      supabaseAdmin
        ...
        .in("status", ["reviewing", "offered", "accepted", "in_progress"]),
      supabaseAdmin
        .from("orders")
        .select("*", { count: "exact", head: true })
        .eq("status", "completed"),
...
const deleteOrder = async (req, res) => {
  try {
    ...
    logger.info(
      `Order ${order.order_number} deleted by admin ${req.user.email}`,
    );
...
const getAllUsers = async (req, res) => {
  try {
    ...
    const usersWithContact = await Promise.all(
      users.map(async (user) => {
        ...
        return {
          ...
          contact_from_order: latestOrder?.form_data
            ? {
                phone:
                  latestOrder.form_data.phone ||
                  latestOrder.form_data.phoneNumber ||
                  null,
                whatsapp:
                  latestOrder.form_data.whatsapp ||
                  latestOrder.form_data.waNumber ||
                  null,
              }
            : null,
        };
    ...
    return sendSuccess(
      res,
      formatPaginatedResponse(usersWithContact, count, page, pageLimit),
    );
  } catch (error) {
    logger.error("Get all users error:", error);
    return sendError(res, "Failed to fetch users", 500);
