...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (profileError) {
      logger.info(`No profile found for email: ${normalizedEmail}`);
      // Still return success to prevent email enumeration
      return sendSuccess(res, null, 'If the email exists, a password reset link has been sent');
    }

    if (profile) {
      logger.info(`Found profile for ${normalizedEmail}, generating token...`);
      ...
      if (tokenError) {
        logger.error('❌ Failed to create password reset token:', tokenError);
        logger.error('Token error details:', JSON.stringify(tokenError, null, 2));
        
        // Check if table exists
        if (tokenError.code === '42P01') {
          logger.error('⚠️ password_resets table does not exist! Please run the SQL migration.');
        }
        
        return sendSuccess(res, null, 'If the email exists, a password reset link has been sent');
      }

      logger.info(`✅ Token created successfully for ${normalizedEmail}`);

      // Send password reset email
      try {
        const resetUrl = `${process.env.FRONTEND_URL}/reset-password?token=${token}`;
        await emailService.sendPasswordResetEmail(normalizedEmail, profile.full_name, resetUrl);
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (tokenError || !resetRecord) {
      logger.warn('Invalid or used reset token attempt');
    ...
    logger.info(`Password reset successful for user: ${resetRecord.user_id}`);
